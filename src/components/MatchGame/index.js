...
// import MatchGamefailure from '../MatchGamefailure'
...
class MatchGame extends Component {
  state = {
    ...
    listOfImages: imagesList[0].thumbnailUrl,
    isGameRunning: true,
    score: 0,
  }
  ...
  timerRun = () => {
    ...
    this.TimerId = setInterval(() => {
      if (counter < 60) {
        ...
        this.setState({isGameRunning: false, score: 0})
      }
    }, 1000)
  }
  ...
  setInitialImages = () => {
    ...
    const randomNumber = Math.ceil(Math.random() * len)
    console.log(randomNumber, len)
    const randomImage = imagesList[randomNumber].imageUrl
    const imgIdDis = imagesList[randomNumber].id
    console.log(imgIdDis, randomImage)

    this.setState({imgUrl: randomImage, imgId: imgIdDis})
  }
  ...
  startAgain = () => {
    this.setState({timer: 0, score: 0, isGameRunning: true})
    this.timerRun()
  }

  render() {
    const {timer, imgUrl, listOfImages, score, isGameRunning} = this.state
    // console.log(listOfImages)
    const filteredImagesList = this.getFilteredImagesList()

    return (
      <div className="match-main-bg-container">
        ...
        {isGameRunning ? (
          <div>
            <div className="main-image-container">
              <img src={imgUrl} alt="avtar" className="main-image" />
            </div>
